...
const ClassShow = ({createReservation, isLoggedIn}) => {
    ...
    useEffect(() => {
      ...
      const URL = process.env.REACT_APP_URL + `api/class/${id}`;
    ...
    const revURL = process.env.REACT_APP_URL + `api/class/${id}`
    ...
    const createReview = async (form) => {
        const createdReview = await fetch(process.env.REACT_APP_URL + `api/class/${id}/review`, {
            method: "POST",
            headers: {
